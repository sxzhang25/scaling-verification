...
def save_weaver_scores(all_test_results, output_path):
    ...
    for idx, row in all_test_results.iterrows():
        sample_probs = row['weaver_sample_scores']  # List of class-1 probabilities
        
        for sample_idx, prob in enumerate(sample_probs):
            info = (idx, sample_idx, prob)
            if prob > 0.5:
...
def train(args):
    ...
    if wandb.run:
        ...
        safe_mkdir(train_file.parent)
        safe_mkdir(test_file.parent)
